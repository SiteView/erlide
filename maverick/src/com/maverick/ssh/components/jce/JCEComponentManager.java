...
{
  SecureRND t;
  static Log d = LogFactory.getLog(JCEComponentManager.class);
    ...
    throws SshException
  {
    try
    {
      return this.t == null ? new SecureRND() : this.t;
    }
    catch (NoSuchAlgorithmException localNoSuchAlgorithmException)
    {
    }
  ...
  protected void initializeKeyExchangeFactory(ComponentFactory paramComponentFactory)
  {
    try
    {
      ...
      if (e("diffie-hellman-group14-sha1", localClass1))
        paramComponentFactory.add("diffie-hellman-group14-sha1", localClass1);
      if (e("diffie-hellman-group1-sha1", localClass2))
        paramComponentFactory.add("diffie-hellman-group1-sha1", localClass2);
      if (e("diffie-hellman-group-exchange-sha1", localClass3))
  ...
  {
    if (d("ssh1-des", Ssh1Des.class))
      paramComponentFactory.add("2", Ssh1Des.class);
    if (d("ssh1-3des", Ssh1Des3.class))
  ...
  private boolean e(String paramString, Class paramClass)
  {
    ...
    try
    {
      String str2 = "SSH-2.0-http://J2SSH_Maverick_1.5.17_rc_";
      ...
      Method localMethod2 = paramClass.getMethod("performClientExchange", new Class[] { String.class, String.class, [B.class, [B.class });
    ...
    catch (InvocationTargetException localInvocationTargetException)
    {
      if ((localInvocationTargetException.getCause() instanceof SshException))
      {
        ...
        {
          d.info("   " + paramString + " will not be supported: " + localInvocationTargetException.getCause().getCause().getMessage());
          return false;
        }
    ...
    d.info("   " + paramString + " will be supported using JCEProvider " + str1);
    return true;
  }
  ...
  private boolean d(String paramString, Class paramClass)
  {
    try
    {
      ...
      if ((localSshCipher instanceof AbstractJCECipher))
        d.info("   " + paramString + " will be supported using JCE Provider " + ((AbstractJCECipher)localSshCipher).getProvider());
      return true;
    }
    catch (Throwable localThrowable)
    {
      d.info("   " + paramString + " will not be supported: " + localThrowable.getMessage());
    }
    return false;
  }
  ...
  private boolean c(String paramString, Class paramClass)
  {
    try
    {
      ...
      if ((localDigest instanceof AbstractDigest))
        d.info("   " + paramString + " will be supported using JCE Provider " + ((AbstractDigest)localDigest).getProvider());
      return true;
    }
    catch (Throwable localThrowable)
    {
  ...
  private boolean b(String paramString, Class paramClass)
  {
    try
    {
      ...
      if ((localSshHmac instanceof AbstractHmac))
        d.info("   " + paramString + " will be supported using JCE Provider " + ((AbstractHmac)localSshHmac).getProvider());
      return true;
    }
    catch (Throwable localThrowable)
    {
