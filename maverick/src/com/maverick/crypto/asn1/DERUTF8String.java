  ...
  implements DERString
{
  ...
  DERUTF8String(byte[] paramArrayOfByte)
  {
    ...
    while (i < paramArrayOfByte.length)
    {
      ...
      if ((paramArrayOfByte[i] & 0xE0) == 224)
      {
        k = (char)((paramArrayOfByte[i] & 0x1F) << 12 | (paramArrayOfByte[(i + 1)] & 0x3F) << 6 | paramArrayOfByte[(i + 2)] & 0x3F);
        i += 3;
      }
      else if ((paramArrayOfByte[i] & 0xC0) == 192)
      {
        k = (char)((paramArrayOfByte[i] & 0x3F) << 6 | paramArrayOfByte[(i + 1)] & 0x3F);
        i += 2;
      }
      else
      {
    ...
    this.hb = new String(arrayOfChar);
  }
  ...
  void encode(DEROutputStream paramDEROutputStream)
    throws IOException
  {
    char[] arrayOfChar = this.hb.toCharArray();
    ...
    paramDEROutputStream.b(12, localByteArrayOutputStream.toByteArray());
  }
}
