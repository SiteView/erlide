...
public class SshX509RsaSha1PublicKey extends Ssh2RsaPublicKey
{
    ...
    throws SshException
  {
    try
    {
      ...
      localByteArrayWriter.writeBinaryString(this.C.getEncoded());
    ...
    throws SshException
  {
    try
    {
      ...
      this.C = ((X509Certificate)localCertificateFactory.generateCertificate(localByteArrayInputStream));
      if (!(this.C.getPublicKey() instanceof RSAPublicKey))
        throw new SshException("Certificate public key is not an RSA public key!", 4);
      this.B = ((RSAPublicKey)this.C.getPublicKey());
    }
    catch (Throwable localThrowable)
    {
