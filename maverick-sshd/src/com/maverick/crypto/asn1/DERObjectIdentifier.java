...
public class DERObjectIdentifier extends DERObject
{
  ...
  private void A(OutputStream paramOutputStream, int paramInt)
    throws IOException
  {
    if (paramInt >= 128)
    {
  ...
  void encode(DEROutputStream paramDEROutputStream)
    throws IOException
  {
    OIDTokenizer localOIDTokenizer = new OIDTokenizer(this.£);
    ByteArrayOutputStream localByteArrayOutputStream = new ByteArrayOutputStream();
    DEROutputStream localDEROutputStream = new DEROutputStream(localByteArrayOutputStream);
    A(localByteArrayOutputStream, Integer.parseInt(localOIDTokenizer.nextToken()) * 40 + Integer.parseInt(localOIDTokenizer.nextToken()));
    while (localOIDTokenizer.hasMoreTokens())
      A(localByteArrayOutputStream, Integer.parseInt(localOIDTokenizer.nextToken()));
    localDEROutputStream.close();
    byte[] arrayOfByte = localByteArrayOutputStream.toByteArray();
    paramDEROutputStream.A(6, arrayOfByte);
  }
  ...
  public boolean equals(Object paramObject)
  {
    if ((paramObject == null) || (!(paramObject instanceof DERObjectIdentifier)))
      return false;
    return this.£.equals(((DERObjectIdentifier)paramObject).£);
  }
}
