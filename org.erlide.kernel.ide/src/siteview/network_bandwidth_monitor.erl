-module (network_bandwidth_monitor).
-compile(export_all).
...
network_bandwidth_monitor(Self, Name)->
	?SETVALUE(browse,[]),
	?SETVALUE(server,[]),
	...
	?SETVALUE(port,161),
	?SETVALUE(deviceType,[]),
	?SETVALUE(duplexState,[]),
	?SETVALUE(indexingMethod,[]),
	?SETVALUE(showRTTraffic,[]),
	?SETVALUE(maxRTDataWindow,24),
	?SETVALUE(maxRTDataVerticalAxis,[]),

	?SETVALUE(name,Name),
	object:super(Self, [Name]).

network_bandwidth_monitor_(Self)->eresye:stop(?VALUE(name)).
