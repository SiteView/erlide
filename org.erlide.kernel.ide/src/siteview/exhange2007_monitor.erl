...
exhange2007_monitor(Self, Name)->
	?SETVALUE(pServer,[]),
	?SETVALUE(pUserDomain,[]),
	?SETVALUE(pMailboxAlias,[]),
	?SETVALUE(pPscFilePath,"C:/Program Files/Microsoft/Exchange Server/Bin/ExShell.psc1"),
	?SETVALUE(browse,[]),

	?SETVALUE(name,Name),
	object:super(Self, [Name]).

exhange2007_monitor_(Self)->eresye:stop(?VALUE(name)).
