...
network_monitor(Self, Name)->

	?SETVALUE(name,Name),
	object:super(Self, [Name]).

network_monitor_(Self)->eresye:stop(?VALUE(name)).
...
%% 	simulated random data
	?SETVALUE(bytesOutPerSecond,[]),
	?SETVALUE(bytesInPerSecond,[]),
	?SETVALUE(connections,[]),
	?SETVALUE(packetErrorsPerSecond,[]),
