...
websphere_monitor(Self, Name)->
	...
	?SETVALUE(password,[]),
	?SETVALUE(version,[]),
	?SETVALUE(webSphereDir,[]),
	?SETVALUE(clientProps,"soap.client.props"),
	?SETVALUE(timeout,60),
	?SETVALUE(trustStore,"C:\\WebSphere\\AppServer\\profiles\\default\\etc\\DummyClientTrustFile.jks"),
	?SETVALUE(trustStorePassword,"WebAS"),
	?SETVALUE(keyStore,"C:\\WebSphere\\AppServer\\profiles\\default\\etc\\DummyClientKeyFile.jks"),
	?SETVALUE(keyStorePassword,"WebAS"),

	?SETVALUE(name,Name),
	object:super(Self, [Name]).

websphere_monitor_(Self)->eresye:stop(?VALUE(name)).
