...
composite_monitor(Self, Name)->
	?SETVALUE(item,[]),
	?SETVALUE(checkSequentially,[]),
	?SETVALUE(delay,5),
	?SETVALUE(deepCheck,[]),
...
composite_monitor_(Self)->eresye:stop(?VALUE(name)).
...
%% 	simulated random data
	?SETVALUE(percentGood,[]),
	?SETVALUE(percentError,[]),
	?SETVALUE(percentWarning,[]),
	?SETVALUE(itemsInError,[]),
	?SETVALUE(itemsInWarning,[]),
	?SETVALUE(itemsInGood,[]),
	?SETVALUE(itemsChecked,[]),
