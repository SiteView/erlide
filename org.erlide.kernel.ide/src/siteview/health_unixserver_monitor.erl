-module (health_unixserver_monitor).
-compile(export_all).
...
health_unixserver_monitor(Self, Name)->
	?SETVALUE(counters,[{"memory","memory"},
...
health_unixserver_monitor_(Self)->eresye:stop(?VALUE(name)).
