-module (network_flow_discard_pkts_rate_monitor).
-compile(export_all).
...
network_flow_discard_pkts_rate_monitor(Self, Name)->
	?SETVALUE(browse,[]),

	?SETVALUE(name,Name),
	object:super(Self, [Name]).

network_flow_discard_pkts_rate_monitor_(Self)->eresye:stop(?VALUE(name)).
