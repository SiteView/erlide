...
script_monitor(Self, Name)->
	?SETVALUE(machine,[]),
	?SETVALUE(pScript,[]),
	?SETVALUE(pParameters,[]),
	?SETVALUE(pValueLabels,[]),
	?SETVALUE(pTimeout,[]),
	?SETVALUE(pRemoteScript,1),
	?SETVALUE(pCacheLife,1),
	?SETVALUE(pExpression,[]),
	?SETVALUE(pMaxMeasurement,[]),
	?SETVALUE(monitor_description,[]),
	?SETVALUE(list_order,[]),

	?SETVALUE(name,Name),
	object:super(Self, [Name]).

script_monitor_(Self)->eresye:stop(?VALUE(name)).
