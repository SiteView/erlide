...
directory_monitor(Self, Name)->
	?SETVALUE(machine,[]),
	?SETVALUE(path,[]),
	?SETVALUE(noRecurse,false),
	?SETVALUE(match,[]),

	?SETVALUE(name,Name),
	object:super(Self, [Name]).

directory_monitor_(Self)->eresye:stop(?VALUE(name)).
...
%% 	simulated random data
	?SETVALUE(size,[]),
	?SETVALUE(exists,[]),
	?SETVALUE(lastmofified,[]),
	?SETVALUE(firstmofified,[]),
	?SETVALUE(fileCount,[]),
	?SETVALUE(permitted,[]),
