-module (database_counter_monitor).
-compile(export_all).
...
database_counter_monitor(Self, Name)->
	?SETVALUE(browse,[]),
	?SETVALUE(connection_url,"localhost:1433/ecc"),
	?SETVALUE(user,"sa"),
	?SETVALUE(password,"123"),
	?SETVALUE(driver,[]),
	?SETVALUE(pquery,[]),
	?SETVALUE(connection_timeout,60),
	?SETVALUE(query_timeout,60),
	?SETVALUE(divisor_query,[]),
	?SETVALUE(no_cumulative,[]),
	?SETVALUE(no_divisor,[]),

	?SETVALUE(name,Name),
	object:super(Self, [Name]).

database_counter_monitor_(Self)->eresye:stop(?VALUE(name)).
