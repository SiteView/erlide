...
web_script_monitor(Self, Name)->
	?SETVALUE(file,[]),
	?SETVALUE(browser,[]),
	?SETVALUE(installDirectory,[]),
	?SETVALUE(server,"127.0.0.1"),
	?SETVALUE(port,4444),

	?SETVALUE(name,Name),
	object:super(Self, [Name]).

web_script_monitor_(Self)->eresye:stop(?VALUE(name)).
