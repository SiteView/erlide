...
mqstatus_monitor(Self, Name)->
	?SETVALUE(serverHostName,[]),
	?SETVALUE(serverPortNumber,"1414"),
	?SETVALUE(channel,[]),
	?SETVALUE(queueMgr,[]),
	?SETVALUE(altqueueMgr,[]),
	?SETVALUE(useMqStatCodes,[]),
	?SETVALUE(username,[]),
	?SETVALUE(password,[]),
...
mqstatus_monitor_(Self)->eresye:stop(?VALUE(name)).
