...
new_port_monitor(Self, Name)->
	?SETVALUE(host,[]),
	?SETVALUE(time,60),
	?SETVALUE(message_send,[]),
	?SETVALUE(message_receive,[]),
	?SETVALUE(autoDiscovery,[]),
	?SETVALUE(port,[]),

	?SETVALUE(name,Name),
	object:super(Self, [Name]).

new_port_monitor_(Self)->eresye:stop(?VALUE(name)).
