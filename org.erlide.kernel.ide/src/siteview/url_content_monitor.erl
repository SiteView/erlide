...
url_content_monitor(Self, Name)->
	?SETVALUE(url,[]),
	?SETVALUE(contentMatch,[]),
	?SETVALUE(timeout,60),
	?SETVALUE(errorContent,[]),
	?SETVALUE(checkContent,[]),
	?SETVALUE(userName,[]),
	?SETVALUE(authorizationpassword,[]),
	?SETVALUE(domain,[]),
	?SETVALUE(whenToAuthenticate,[]),
	?SETVALUE(proxyUserName,[]),
	?SETVALUE(proxyPassword,[]),
	?SETVALUE(postData,[]),
	?SETVALUE(errorOnRedirect,[]),
	?SETVALUE(urlEncoding,[]),
	?SETVALUE(monitorRunCount,[]),
	?SETVALUE(httpVersion10,[]),
	?SETVALUE(retries,[]),
	?SETVALUE(acceptAllUntrustedCerts,[]),
	?SETVALUE(acceptInvalidCerts,[]),
	?SETVALUE(encodePostData,[]),
...
url_content_monitor_(Self)->eresye:stop(?VALUE(name)).
