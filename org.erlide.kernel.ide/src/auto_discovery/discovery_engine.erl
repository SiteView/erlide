...
?PATTERN(newnode_pattern) -> {discover_rule_engine, read, {newnode,'_','_','_','_'}}. %% node,port, discovered_by, timestamp
...
newnode_action(Self,EventType,Pattern,State) -> 
	...
	NodeInfoLen = length(eresye:query_kb(discover_rule_engine, {Node,is,'_'})),
	if 	
