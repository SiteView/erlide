...
handle_call({create_machine, Machine, HostName}, _From, State) ->
	...
	DumbStr = lists:sublist(Machine#machine.host,4),
	io:format("update_machine Machine#machine.host =~p~n", [Machine#machine.host]),
	Reply =
    if 
        IsIP =:= false , DumbStr =/= "DUMB" -> 
...
handle_call({update_machine,Mach, HostName}, _From, State) ->
	...
	DumbStr = lists:sublist(Mach#machine.host,4),
	%---io:format("update_machine Mach#machine.host =~p~n", [Mach#machine.host]),
    Reply = 
    if 
        IsIP =:= false ,DumbStr =/= "DUMB"->
            {error, 'ip format error'};
                ...
                {ok, [{Id, Val}]} ->
                    case Val of
                        Vlist when erlang:is_list(Vlist) ->
                            io:format("UserTag: ~p~n", [string:len(Vlist)]),
...
writeDefaultTag_T() ->
    ...
    deefultTag_Method([{"SSH", "SSH"}, {"Telnet", "Telnet"}, {"WMI", "WMI"},{"Snmp", "Snmp"}]),
