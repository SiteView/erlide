...
getCounterValues(Host,User,Passwd, EventType, EventID) ->
	...
	TT = filterCount(rpc_proxy:call(THIS:get_wmi_node(), wmic, wmic, [Host,User,Passwd,TWql])),
	II = filterCount(rpc_proxy:call(THIS:get_wmi_node(), wmic, wmic, [Host,User,Passwd,IWql])),
