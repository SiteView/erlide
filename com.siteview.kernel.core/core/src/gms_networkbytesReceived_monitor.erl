...
getCounterValues(Host,User,Passwd,Counters)->
    ...
    case rpc_proxy:call(THIS:get_wmi_node(), wmic, wmic, [Host,User,Passwd,Ssql++Ssql1]) of
		{ok, Result} ->
...
getInstances(Host,User,Passwd)->
	...
	case rpc_proxy:call(THIS:get_wmi_node(), wmic, wmic, [Host,User,Passwd,Ssql]) of
            {ok, Result} ->
				case Result of
					[{_SQL,{_,empty}}]->
			          [];
